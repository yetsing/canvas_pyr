...
use crate::{
    error::SkError,
    sk::{
        Color, ConicGradient, Gradient as SkGradient, LinearGradient, RadialGradient, Shader,
        TileMode, Transform,
    },
};
...
impl Gradient {
    ...
    pub(crate) fn get_shader(
        ...
        match self {
            Self::Linear(linear_gradient) => Ok(Shader::new_linear_gradient(&LinearGradient {
            ...
            Self::Radial(radial_gradient) => {
                ...
                Ok(
                    Shader::new_radial_gradient(&new_radial_gradient).ok_or_else(|| {
                        SkError::Generic("Get shader of radial gradient failed".to_owned())
                    })?,
                )
            }
            Self::Conic(conic_gradient) => {
                ...
                Ok(
                    Shader::new_conic_gradient(&new_conic_gradient).ok_or_else(|| {
                        SkError::Generic("Get shader of radial gradient failed".to_owned())
                    })?,
                )
            }
...
#[pymethods]
impl CanvasGradient {
    #[pyo3(name = "addColorStop")]
    pub fn add_color_stop(&mut self, offset: f64, color: String) -> PyResult<()> {
        if color.is_empty() {
            return Ok(());
        }
        ...
        let color = CSSColor::parse(&mut parser).map_err(|e| {
            PyValueError::new_err(format!("Parse color [{color_str}] error: {e:?}"))
        })?;
        let skia_color = match color {
            CSSColor::CurrentColor => {
                return Err(PyValueError::new_err(
                    "Gradient stop color should not be `currentcolor` keyword",
                ));
            }
            ...
            _ => {
                return Err(PyValueError::new_err("Unsupported color format"));
            }
        };
        self.0.add_color_stop(offset as f32, skia_color);
        Ok(())
    }
}
